...
impl OpenAiCompatibleProvider {
    ...
    /// When true, user messages with `[IMAGE:...]` markers are serialized as OpenAI-style
    /// multimodal `content` parts (text + `image_url`). Requires `supports_vision` and
    /// stays false when `merge_system_into_user` is set (e.g. MiniMax).
    #[inline]
    fn user_image_parts_enabled(&self) -> bool {
        self.supports_vision && !self.merge_system_into_user
    }
...
impl OpenAiCompatibleProvider {
    ...
    /// OpenAI-compatible SSE stream for a fully built chat-completions `messages` payload.
    fn stream_chat_completions_request(
        &self,
...
impl Provider for OpenAiCompatibleProvider {
    ...
    async fn chat_with_system(
        ...
        if self.merge_system_into_user {
            ...
            messages.push(Message {
                role: "user".to_string(),
                content: Self::to_message_content("user", message, self.user_image_parts_enabled()),
            });
        }
    ...
    fn stream_chat_with_history(
        ...
        self.stream_chat_completions_request(api_messages, model, temperature, options)
    }
    ...
    fn stream_chat_with_system(
        ...
        self.stream_chat_completions_request(messages, model, temperature, options)
    }
